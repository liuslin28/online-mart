<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item, index) in addressListFilter" v-bind:class="{'check': checkIndex == index}" @click="checkIndex = index; selectAddId = item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-default" v-if="item.isDefault" @click="delAddressConfirm(item.addressId)">删除地址</div>
                </li>
                <li class="addr-new">
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascript:;" @click="expandAddress" v-bind:class="{'open': limitAdd > 3}">
                更多地址
                <i class="i-up-down">
          ...
          <!-- shipping method-->
          <div class="page-title-normal checkout-title">
            <h2><span>配送方法</span></h2>
          </div>
          <div class="shipping-method-wrap">
            <div class="shipping-method">
              <ul>
                <li class="check">
                  <div class="name">普通物流</div>
                  <div class="price">免运费</div>
          ...
          <div class="next-btn-wrap">
            <router-link class="btn btn--m btn--red" v-bind:to="{path:'orderConfirm',query:{addressId:selectAddId}}">继续</router-link>
          </div>
        </div>
      </div>
    </div>
    <modal :mdShow="ismdShow" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a href="javascript:;" class="btn btn--m" @click="delDefault">确认</a>
        <a href="javascript:;" class="btn btn--m btn--red" @click="ismdShow = false">取消</a>
      </div>
    </modal>
...
<script>
  ...
  export default {
      ...
      computed: {
          addressListFilter() {
              return this.addressList.slice(0, this.limitAdd);
          }
      },
      mounted() {
          this.init();
...
//          更多地址
          expandAddress() {
              if (this.limitAdd === 3) {
                  this.limitAdd = this.addressList.length;
              } else {
                  this.limitAdd = 3;
              }
          },
//          设置默认地址
          setDefault(addressId) {
              axios.post('/users/setDefault', {
                  ...
                  let res = response.data;
                  if (res.status === '0') {
                      console.log('set default is done');
                      this.init();
                  }
              });
          },
